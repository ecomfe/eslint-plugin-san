...
const NON_STANDARD_KNOWN_NODES = new Set(['ExperimentalRestProperty', 'ExperimentalSpreadProperty']);
const LT_CHAR = /[\r\n\u2028\u2029]/;
const LINES = /[^\r\n\u2028\u2029]+(?:$|\r\n|[\r\n\u2028\u2029])/g;
    ...
    function isBeginningOfElement(token, belongingNode) {
        ...
        while (node != null && node.parent != null) {
            ...
            if (parent.type === 'CallExpression' || parent.type === 'NewExpression') {
                const openParen = /** @type {Token} */ (tokenStore.getTokenAfter(parent.callee, isNotRightParen));
                return parent.arguments.some(
                    param => getFirstAndLastTokens(param, openParen.range[1]).firstToken.range[0] === token.range[0]
                );
            }
            if (parent.type === 'ArrayExpression') {
                return parent.elements.some(
                    element => element != null && getFirstAndLastTokens(element).firstToken.range[0] === token.range[0]
                );
            }
    ...
    return processIgnores({
        ...
        VFilterSequenceExpression(node) {
            ...
            for (const filter of node.filters) {
                tokens.push(tokenStore.getTokenBefore(filter, isPipeOperator), tokenStore.getFirstToken(filter));
            }
        ...
        VForExpression(node) {
            ...
            const inToken = /** @type {Token} */ (tokenStore.getTokenAfter(lastOfLeft, isNotRightParen));
        ...
        'AssignmentExpression, AssignmentPattern, BinaryExpression, LogicalExpression'(node) {
            ...
            const opToken = /** @type {Token} */ (tokenStore.getTokenAfter(node.left, isNotRightParen));
        ...
        'BreakStatement, ContinueStatement, ReturnStatement, ThrowStatement'(node) {
            if (
                ...
                ((node.type === 'BreakStatement' || node.type === 'ContinueStatement') && node.label != null)
            ) {
                const firstToken = tokenStore.getFirstToken(node);
                const nextToken = tokenStore.getTokenAfter(firstToken);
        ...
        ConditionalExpression(node) {
            const prevToken = tokenStore.getTokenBefore(node);
            const firstToken = tokenStore.getFirstToken(node);
            const questionToken = /** @type {Token} */ (tokenStore.getTokenAfter(node.test, isNotRightParen));
            const consequentToken = tokenStore.getTokenAfter(questionToken);
            const colonToken = /** @type {Token} */ (tokenStore.getTokenAfter(node.consequent, isNotRightParen));
        ...
        ExportNamedDeclaration(node) {
            ...
            if (node.declaration) {
                ...
                if (!firstSpecifier || firstSpecifier.type === 'ExportSpecifier') {
                    ...
                    const rightParenToken = /** @type {Token} */ (tokenStore.getLastToken(node, isRightBrace));
        ...
        'ForInStatement, ForOfStatement'(node) {
            ...
            const awaitToken =
                (node.type === 'ForOfStatement' && node.await && tokenStore.getTokenAfter(forToken)) || null;
            const leftParenToken = tokenStore.getTokenAfter(awaitToken || forToken);
            const leftToken = tokenStore.getTokenAfter(leftParenToken);
            const inToken = /** @type {Token} */ (tokenStore.getTokenAfter(leftToken, isNotRightParen));
        ...
        IfStatement(node) {
            ...
            if (node.alternate != null) {
                const elseToken = /** @type {Token} */ (tokenStore.getTokenAfter(node.consequent, isNotRightParen));
        ...
        'MemberExpression, MetaProperty'(node) {
            ...
            if (node.type === 'MemberExpression' && node.computed) {
                const leftBracketToken = /** @type {Token} */ (tokenStore.getTokenBefore(node.property, isLeftBracket));
        ...
        'MethodDefinition, Property'(node) {
            ...
            let lastKeyToken;
            if (node.computed) {
                const keyLeftToken = /** @type {Token} */ (tokenStore.getFirstToken(node, isLeftBracket));
        ...
        SwitchStatement(node) {
            ...
            const leftBraceToken = /** @type {Token} */ (tokenStore.getTokenAfter(node.discriminant, isLeftBrace));
        ...
        Program(node) {
            ...
            const isScriptTag =
                firstToken != null && firstToken.type === 'Punctuator' && firstToken.value === '<script>';
            const baseIndent = isScriptTag ? options.indentSize * options.baseIndent : 0;
        ...
        "VElement[parent.type!='VElement']"(node) {
            const result = context.getSourceCode();
            let startIndent = 0;
            if (result.ast && Array.isArray(result.ast.templateBody)) {
                const templateBody = result.ast.templateBody;
                for (let template of templateBody) {
                    if (template.hasOwnProperty('templateLoc') && template.hasOwnProperty('templateRange')) {
                        const nodeLine = node.loc.start.line;
                        const templateLine = template.templateLoc.start.line;
                        const [nodeStart, nodeEnd] = node.range;
                        const [start, end] = template.templateRange;
                        if (nodeStart > start && nodeEnd < end) {
                            let column = template.templateLoc.start.column;
                            if (nodeLine === templateLine) {
                                context.report({
                                    node: node,
                                    loc: node.loc,
                                    message: 'Root tag must in the new line in template literal.',
                                    fix: (fixer) => {
                                        return [
                            ...
                            startIndent = column + options.indentSize;
                            break;
                        }
                    }
                }
            }
            processTopLevelNode(node, startIndent);
        },
        ...
        ":matches(Program, VElement[parent.type!='VElement']):exit"(node) {
            ...
            let temp = tokenStore.getTokens(node, ITERATION_OPTS);
            ...
            for (const token of temp) {
                if (tokensOnSameLine.length === 0 || tokensOnSameLine[0].loc.start.line === token.loc.start.line) {
                    ...
                    isBesideMultilineToken =
                        /** @type {Token} */ (last(tokensOnSameLine)).loc.end.line === token.loc.start.line;
