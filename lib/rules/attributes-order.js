...
function getAttributeName(attribute, sourceCode) {
    if (attribute.directive) {
        if (attribute.key.name.name === 'bind') {
            return attribute.key.argument ? sourceCode.getText(attribute.key.argument) : '';
        } else {
...
function getAttributeType(attribute, sourceCode) {
    ...
    if (attribute.directive) {
        ...
        propName = attribute.key.argument ? sourceCode.getText(attribute.key.argument) : '';
    } else {
    ...
    if (propName === 'is') {
        ...
        return ATTRS.GLOBAL;
    } else if (propName === 'ref' || propName === 'key' || propName === 'slot' || propName === 'slot-scope') {
        return ATTRS.UNIQUE;
    } else {
...
function create(context) {
        ...
        context.report({
            ...
            fix(fixer) {
                ...
                return shiftAttrs.map((attr, i) => {
                    const text =
                        attr === previousNode ? sourceCode.getText(node) : sourceCode.getText(shiftAttrs[i - 1]);
    ...
    return utils.defineTemplateBodyVisitor(context, {
        ...
        VAttribute(node) {
            ...
            if (!state || (state.currentPosition <= getPosition(node, attributePosition, sourceCode) && inAlphaOrder)) {
                state = {
...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            url: 'https://ecomfe.github.io/eslint-plugin-san/rules/attributes-order.html'
        },
        fixable: 'code',
        schema: {
            type: 'array',
