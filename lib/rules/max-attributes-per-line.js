...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            url: 'https://ecomfe.github.io/eslint-plugin-san/rules/max-attributes-per-line.html'
        },
            ...
            attributes.forEach((prop, i) => {
                context.report({
                    ...
                    fix(fixer) {
                        ...
                        const prevToken = /** @type {Token} */ (template.getTokenBefore(prop, {
                            filter: token => token.type !== 'HTMLWhitespace'
                        }));

                        /** @type {Range} */
                        const range = [prevToken.range[1], prop.range[0]];
                        let prevAttr = prop.parent;
                        if (prevAttr.attributes) {
                            prevAttr = prevAttr.attributes[0];
                        }
                        let indent = 0;
                        if (prevAttr) {
                            indent = prevAttr.loc.start.column;
                            indent < 0 && (indent = 0); 
                        }
