...
function extractTopLevelDocumentFragmentComments(documentFragment) {
    ...
    return documentFragment.comments.filter(comment =>
        elements.every(element => comment.range[1] <= element.range[0] || element.range[1] <= comment.range[0])
    );
}
...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            categories: ['base'],
            url: 'https://ecomfe.github.io/eslint-plugin-san/rules/comment-directive.html'
        },
        schema: [
            {
                type: 'object',
    ...
    create(context) {
        ...
        function checkTemplateBody(templateBody) {
            // Send directives to the post-process.
            for (const comment of templateBody.comments) {
        ...
        return {
            Program(node) {
                if (Array.isArray(node.templateBody)) {
                    for (const templateBody of node.templateBody) {
                        checkTemplateBody(templateBody);
                    }
                } else if (node.templateBody) {
                    checkTemplateBody(node.templateBody);
                }
                // only for .san file
                if (documentFragment) {
                    // Send directives to the post-process.
